...
public class ClasspathURLConverterJBoss7Dot1 implements ClasspathURLConverter {
		...
		if (url != null && url.getProtocol().startsWith("vfs")) {
			...
			try {
				...
				if (urlString.contains(".jar")) {
					...
					File physicalWarDir = (File) invokerGetter(
							jBossVirtualWarDir, "getPhysicalFile");
				...
				// Otherwise, ask the VFS what the physical URL is...
					...
					// Use reflection so that we don't need JBoss in the
					// classpath at compile time.
					File physicalFile = (File) invokerGetter(jBossVirtualFile,
							"getPhysicalFile");
	...
	private Object invokerGetter(Object target, String getter)
