...
public class X11 {
    ...
    static {
        ...
        try {
            System.out.println("Loading x11 native library...");
            System.loadLibrary("x11");
            native_library_loaded = true;
            System.out.println("Loaded x11 successfully.");
        } catch (UnsatisfiedLinkError x) {
            System.out.println("Loading x11 failed: " + x.getMessage());
        } catch (Exception e) {
            native_library_loaded = false;
            System.out.println("Loading x11 failed: " + e.getMessage());
        }
    }
    ...
    public static byte[] x11Digest(byte[] input) {
        ...
        try {
            ...
            /*
             * long start = System.currentTimeMillis(); byte [] result = x11_native(input); long
             * end1 = System.currentTimeMillis(); byte [] result2 = x11(input); long end2 =
             * System.currentTimeMillis(); System.out.println("x11: native {} / java {}",
             * end1-start, end2-end1); return result;
             */
        } catch (Exception e) {
            return null;
        } finally {
            // long time = System.currentTimeMillis()-start;
            // System.out.println("X11 Hash time: {} ms per block", time);
        }
    }
